...
def get_menu_choice(options, code=None, isclean=True, give_key_str=False):
    shortcuts = scan_short_cuts(options)  # scan for shortcuts
    selected_index = 0
    # print(shortcuts)
    while True:
        show_menu(code, options, selected_index, isclean)
        key = get_key()
        if key == 'enter':  # Enter key to select
            ...
            show_menu(code, options, shortcuts[key], isclean)  # show selected option when using shortcut
            if (give_key_str):
                return key
            else:
                return shortcuts[key]
...
def show_menu(code, options, selected_index, isclean):
    if isclean:
        show_clean_menu(code, options, selected_index)
    else:
...
def show_clean_menu(code, options, selected_index):
    os.system("cls" if os.name == "nt" else "clear")
    if code:
        [eval(_) for _ in code]
    for i, option in enumerate(options):
        if i == selected_index:
            print(f"> {option}")
        else:
